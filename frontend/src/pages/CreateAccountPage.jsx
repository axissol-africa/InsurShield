...
import { DEMO_CUSTOMER_ACCOUNT, useStore } from '../store/useStore';
...
export default function CreateAccountPage() {
  ...
  const next = searchParams.get('next') || '/select-insurers';
  ...
  const submitLogin = event => {
    event.preventDefault();
    if (!authenticateCustomer(identifier, details.password)) return setError('We could not sign you in with those details. Check them, try again, or reset your password.');
  ...
  return <main className="flex min-h-[calc(100vh-64px)] items-center justify-center px-4 py-10"><motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl">
    <header className="bg-primary px-7 py-6 text-white"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-white/70">Your InsurShield account</p><h1 className="mt-1 text-[27px] font-bold">{stage === 'otp' ? 'Verify your identity' : title}</h1><p className="mt-2 text-[13px] text-white/80">Guests can explore freely. An account is required to request quotes, manage claims and renew policies.</p></header>
    <div className="p-6 md:p-7">
      ...
      {stage === 'success' && <Outcome icon="check_circle" success title="Your account is ready" body="Your account and required consent are set up. You can now request quotes, manage claims and renew a policy." action="Continue" onAction={() => navigate(next, { replace: true })} />}
    </div>
...
function Notice({ text }) { return text ? <p className="rounded-xl bg-green-50 px-4 py-3 text-[12px] font-medium text-green-700">{text}</p> : null; }
